        ...
        public Respostory ConnectServer(OPCDto dto)
        {
            ...
            foreach (string item in ServerDic.Keys)
            {
                if(item.Contains(url))
                   return new Respostory(true, ServerDic[url].mItempNameList);
            }
                ...
                switch (dto.ItemMethod)
                {
                    case UpdateMethod.Add:
                        ...
                        if(res)
                        {
                            ...
                            ServerDic[url].Subscription.DataChanged += OnDataChange;
                            //返回整个键值对
                            return new Respostory(true, (object)dto.ItemName);
                        }
                        else
                        {
      ...
      /// <returns></returns>
        public List<ItemValue> GetValue()
        {
            ...
            foreach (var item in ServerDic)
            {
                ...
                {
                    if(item1.Value.Value.Value!=null)
                    {
        ...
        public Respostory Disconnect(OPCDto dto)
        {
            ...
            try
            {
                foreach (string item in ServerDic.Keys)
                {
                    if (ServerDic[item].Subscription != null && ServerDic[item].Subscription.Items != null && item == url)
                    {
        ...
        public void OnDataChange(object subscriptionHandle, object requestHandle, ItemValueResult[] values)
        {
            ...
            foreach (ItemValueResult item in values)
            {
                ...
                {
                    //如果句柄相同,找到对应的key值.
                    if (dicNode.Value.Subscription!=null && dicNode.Value.SubscriptionState!=null && dicNode.Value.Subscription.ClientHandle == subscriptionHandle)
                    {
        ...
        public List<ItemValue> GetValues()
        {
            lock(this)
            {
                ...
                foreach (var item in ServerDic)
                {
                    if (item.Value.ItemsDic != null && item.Value.ItemsDic.Count > 0)
                    {
                        foreach (var item1 in item.Value.ItemsDic)
                        {
                            list.Add(item1.Value.Value);
